...
public class MeanItemBasedItemRecommender extends AbstractItemBasedItemRecommender {
    ...
    private ResultList recommendItems(int n, LongSet items) {
        ...
        while(it.hasNext()){
            long movieId= (long) it.next();
            try{
                double score=model.getMeanRating(movieId);
                Result res = Results.create(movieId, score);
                results.add(res);
            }
            catch(IllegalArgumentException e){
                logger.info("No rating available for the movie : "+ movieId + " " +e.getMessage());
            }
        ...
        if(n<0){
            ...
            Comparator<Result> comp=new Comparator<Result>() {
                @Override
